...
func (c *HelpCommand) Execute(_ context.Context, _ *CommandContext, args []string) (*CommandResult, error) {
	...
	if len(args) > 0 {
		...
		if !ok {
			return &CommandResult{
				Content: fmt.Sprintf("Lệnh không hợp lệ `%s`, gõ `/help` để xem các lệnh khả dụng.", args[0]),
			}, nil
		}
		return &CommandResult{
	...
	sb.WriteString("**Các lệnh khả dụng**\n\n")
	for _, cmd := range cmds {
		sb.WriteString(fmt.Sprintf("· `/%s` — %s\n", cmd.Name(), cmd.Description()))
	}
	sb.WriteString("\nGõ `/help <tên lệnh>` để xem cách dùng chi tiết")
