...
func (c *InfoCommand) Description() string { return "Xem thông tin và khả năng của trợ lý hiện tại" }
...
func (c *InfoCommand) Execute(ctx context.Context, cmdCtx *CommandContext, _ []string) (*CommandResult, error) {
	...
	if cmdCtx.CustomAgent == nil {
		sb.WriteString("\nChưa gắn trợ lý nào, gõ `/help` để xem các lệnh khả dụng.")
	...
	if cmdCtx.CustomAgent.IsAgentMode() {
		sb.WriteString("\n🧠 **Chế độ Agent**\n")
		sb.WriteString("Hỗ trợ suy luận nhiều bước, gọi công cụ (ReAct)\n")
	} else {
		sb.WriteString("\n🧠 **Chế độ Agent**\n")
		sb.WriteString("Trả lời trực tiếp dựa trên kho tri thức (RAG)\n")
	}
	...
	if cfg.KBSelectionMode == "all" {
		...
		if err == nil && len(kbs) > 0 {
			...
			sb.WriteString(fmt.Sprintf("  Tổng %d kho (bật tất cả)\n", len(kbs)))
		} else {
			sb.WriteString("  Bật tất cả\n")
		}
		...
		if err == nil {
			...
			sb.WriteString(fmt.Sprintf("  Đã chọn %d kho\n", len(cfg.KnowledgeBases)))
		}
	} else {
	...
	// ── Footer ──
	outputLabel := "Xuất theo luồng (streaming)"
	if cmdCtx.ChannelOutputMode == "full" {
		outputLabel = "Xuất đầy đủ"
	}
