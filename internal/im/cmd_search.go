...
func (c *SearchCommand) Description() string {
	return "Tìm trực tiếp nguyên văn trong kho tri thức (không qua AI tóm tắt), ví dụ: /search chính sách hoàn tiền"
}
...
func (c *SearchCommand) Execute(ctx context.Context, cmdCtx *CommandContext, args []string) (*CommandResult, error) {
	...
	sb.WriteString(fmt.Sprintf("🔍 **Tìm \"%s\"** — thấy %d kết quả\n\n", query, len(results)))
	...
	if len(results) > searchMaxResults {
		sb.WriteString(fmt.Sprintf("_(chỉ hiển thị %d kết quả đầu, tổng %d kết quả)_", searchMaxResults, len(results)))
	}
